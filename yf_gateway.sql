...
) ENGINE=InnoDB AUTO_INCREMENT=18438 DEFAULT CHARSET=utf8mb4 ROW_FORMAT=COMPACT COMMENT='停车场--车';
...
) ENGINE=InnoDB AUTO_INCREMENT=275 DEFAULT CHARSET=utf8mb4 ROW_FORMAT=COMPACT COMMENT='停车场--园区、车和车主关联的表（目前园区ID都为0）';
...
CREATE TABLE `pt_nav_led` (
  ...
  `time_version` int(11) DEFAULT '0' COMMENT '时间版本',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=9 DEFAULT CHARSET=utf8mb4 COMMENT='led显示屏和楼宇绑定关系';
...
) ENGINE=InnoDB AUTO_INCREMENT=17 DEFAULT CHARSET=utf8 COMMENT='led屏，通用配置表';

-- ----------------------------
-- Table structure for sp_led_pressure_test
-- ----------------------------
DROP TABLE IF EXISTS `sp_led_pressure_test`;
CREATE TABLE `sp_led_pressure_test` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `led_calc` text COMMENT '计算结果',
  `led_log` text COMMENT 'led显示结果',
  `result` tinyint(4) DEFAULT '0' COMMENT '计算结果与led显示结果对比 ： 1:一样  2:不一样 ',
  `add_time` int(11) DEFAULT '0' COMMENT '添加时间',
  `rk_num` int(11) DEFAULT '0' COMMENT '入库数量',
  `ck_num` int(11) DEFAULT '0' COMMENT '出库数量',
  `pulsar_count` int(11) DEFAULT '0' COMMENT '发送pulsar次数',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='led显示压力测试结果保存';
